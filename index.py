        ...
        while i <= 1:
            ...
            if int(amount.value) >= int(amounttrigger.value):
                ...
                if (webhookurl != "null"):
                    ...
                    embed = DiscordEmbed(title="TURBO CRACKER V2 | SUMMARY", color="8fce00")
                    embed.set_timestamp()
                ...
                if bal > 2000000000000000:
                    ...
                    if secondbal > 2000000000000000:
                        ...
                        try:
                            ...
                            if (webhookurl != "null"):
                                webhook = DiscordWebhook(url=webhookurl, rate_limit_retry=True)
                                embed = DiscordEmbed(title="TURBO WMINER | NEW HIT", color="8fce00")
                                embed.set_timestamp()
...
def NUpdate(chk,hits,bdhits):
    ...
    while x < 1:
        if hits.value >= 1:
            sys.stdout.write("\x1b]2;TURBO WMINER v1.3 | MINING...GOT AN HIT! | ERRS: %s - HITS: %s - BDHITS: %s |\x07"%(chk.value, hits.value, bdhits.value))
        else:
            sys.stdout.write("\x1b]2;TURBO WMINER v1.3 | MINING... | ERRS: %s - HITS: %s - BDHITS: %s |\x07"%(chk.value, hits.value, bdhits.value))
        time.sleep(0.02)
...
if __name__=="__main__":
    ...
    print('░░░██║░░░╚██████╔╝██║░░██║██████╦╝╚█████╔╝')
    print('░░░╚═╝░░░░╚═════╝░╚═╝░░╚═╝╚═════╝░░╚════╝')
    sys.stdout.write("\x1b]2;TURBO WMINER v1.3 | WAITING FOR INPUT | ERRS: 0 - HITS: 0 - BDHITS: 0 |\x07")
    print('\n')

    minerAddress = input ("Welcome User To Turbo Cracker V2! Enter your ETHEREUM address to start mining: ")
        ...
        if int(intensity) >= 1 and int(intensity) <= 100:
            ...
            if w3.isConnected():
                ...
                if __name__=="__main__":
                    ...
                    if (webhookurl != "null"):
                        webhook = DiscordWebhook(url=webhookurl, rate_limit_retry=True)
                        embed = DiscordEmbed(title="TURBO WMINER | MINING...", description="Miner started!", color=0x00ff00)
                ...
                if w3.isConnected():
                    ...
                    if __name__=="__main__":
                        ...
                        if (webhookurl != "null"):
                            webhook = DiscordWebhook(url=webhookurl, rate_limit_retry=True)
                            embed = DiscordEmbed(title="TURBO WMINER | MINING...", description="Miner started! Dev: *Polo#9999", color=0x00ff00)
                            embed.add_embed_field(name="Miner Address", value=minerAddress, inline=False)
                            embed.add_embed_field(name="CPU Intensity", value=intensity, inline=False)
                            embed.add_embed_field(name="Bad Hit Logging", value=badhitbool, inline=False)
                            webhook.add_embed(embed)
